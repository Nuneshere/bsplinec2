...
function pontoExtremoEsq(i){
    var corX = ( (esquerdoExtremoEsq(i) * points[i-1].x ) + (direitoExtremoEsq(i)* points[i].x ) ) ; 
    var corY = ( (esquerdoExtremoEsq(i) * points[i-1].y ) + (direitoExtremoEsq(i)* points[i].y ) ) ; 
    var ponto = {x:corX, y:corY}; 
    return ponto;
}

function pontoExtremoDir(i){ //3*i-2
    var corX = ( (esquerdaExtremoDir(i) * points[i-1].x ) + (direitaExtremoDir(i)* points[i].x ) ) ; 
    var corY = ( (esquerdaExtremoDir(i) * points[i-1].y ) + (direitaExtremoDir(i)* points[i].y ) ) ; 
    var ponto = {x:corX, y:corY}; 
    return ponto;
}
